  ...
  process {
    ...
    foreach ($shop in $shops) {
      ...
      if (!(Test-Path -Path $itemPath -ErrorAction SilentlyContinue)) {
        $siteBranch = "Branches/Feature/Kortex/Practical Info/Shop"
      ...
      $pageDesignPath = "master:/content/Klepierre/$($RegionName)/$($siteName)/Presentation/Page Designs/Shop Details"
      ...
      $item["Shop Title"] = $shop["Shop Name"]
      ...
      if ($shop["Banner image"] -ne "") {
        ...
        if ($null -ne $bannerImageItem) {
          ...
          $item["Shop Image"] = '<image mediaid="' + $BannerImageID + '" alt="' + $bannerImageItem["Alt"] + '" height="" width="" hspace="" vspace="" />'
        }
      }
      ...
      foreach($openingHourItem in $openingHourItems){
        ...
        $opening = "00010101T" + $opening + "00"
        $closing = $shop[$itemName+' Closing']
        $closing = $closing.Replace(":","")
        $closing = "00010101T" + $closing + "00"
