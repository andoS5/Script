...
function Export-Stores {
    ...
    process {
        ...
        $outputFilePath = "$($AppPath)/$($RegionName)_StoresBrand_$($MallName).csv"
        ...
        foreach ($shopChildItem in $shopChildItems) {
            ...
            $shopNameItem = Get-Item -Path master: -ID "$shopID" -Language $Language
            $shopName = $shopNameItem.Name
            $BrandID = $shopNameItem["Brand"]
            $BrandItem = Get-Item -Path master: -ID "$BrandID" -Language $Language
            ...
            $obj | Add-Member -MemberType NoteProperty -Name "ShopName" -Value  $shopName
        ...
        $array | Select-Object RegionName, MallName, ShopName, ShopTitle, BrandName | Export-Csv -Encoding UTF8 -notypeinformation -Path $outputFilePath
        Try {
