        ...
        if( $regionName -eq "France"){
            foreach($repo in $Repository){
                ...
                foreach($ch in $child){
                    ...
                    # $subCat = $ch["Sub Category"]
                    $cat = Get-Item -Path master: -ID  "$category" -Language $version
                    # $sub = Get-Item -Path "master:/sitecore/content/" -ID  "$subCat" -Language $version
                    Write-Host "$regionName/$MallName/$shopName ==> $($cat.Name)"
                }
            }
        }
    }
