...
foreach($f in $file){
    ...
    $type = $f."Category 1"
    $tpl = if($type -eq "Restaurants") {"{A496118C-D0EF-440D-8DF5-5DEFE47E662E}"}else{"{80D19FA2-B1EF-4E32-93BD-4B2C650A126F}"}
    # Write-Host "template " $tpl
    # Continue
    $path = "/sitecore/content/Klepierre/$Region/$MallName/Home/Shop Repository/$itemName"
    if(!(Test-Path -Path $path -ErrorAction SilentlyContinue)){
        Write-Host "Create New resto $itemName"
        New-Item -Path $path -ItemType $tpl -Language $version | Out-Null
    }
    ...
    $pathImage = "/sitecore/media library/Project/Klepierre/$Region/$MallName/Stores"
    ...
    #Create Brand
    $pathBrand = "/sitecore/content/Klepierre/$Region/Brand Repository/$itemName"
    ...
    if($null -ne $itemName){
        ...
        $item["Twitter Url"] = $twitVal
        $item["Page Design"] = "{FF730966-A1F0-4CD5-8F9E-15BA5BB5D636}"
        ...
        if($tileImage -ne $null){
          if($type -eq "Restaurants"){
            $item["Shop Image"] = '<image mediaid="' + $tileImage.ID + '" alt="' + $tileImage.Name+ '" height="" width="" hspace="" vspace="" />'
          }else{
        ...
        #Update Hour 
        $pathHourRepo = "/sitecore/content/Klepierre/$Region/$MallName/Home/Shop Repository/$($itemName)/Opening Hours/"
